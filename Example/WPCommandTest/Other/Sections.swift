...
nonisolated enum SectionLayout: Sendable {
    ...
    case fullWidth(height: NSCollectionLayoutDimension)
    /// Item 宽度占满 Section，高度由 Cell 的 Auto Layout 内容自适应。
    case selfSizingFullWidth(estimatedHeight: CGFloat)
    case carousel(itemWidth: CGFloat, itemHeight: CGFloat, scrolling: SectionScrollingBehavior)
    /// 横向滚动 Item 固定宽度，高度由 Cell 内容自适应。
    case selfSizingCarousel(itemWidth: CGFloat, estimatedHeight: CGFloat, scrolling: SectionScrollingBehavior)
    /// 横向分页卡片占满 Section 可用宽度，高度按宽高比计算。
    case fullWidthPaging(aspectRatio: CGFloat, scrolling: SectionScrollingBehavior)
    ...
    /// 固定列数，并根据 Item 宽高比自动计算高度的纵向网格。
    case fixedAspectGrid(columns: Int, aspectRatio: CGFloat, spacing: CGFloat)
}
...
nonisolated struct Section<ID: Hashable & Sendable>: Hashable, @unchecked Sendable {
    ...
    func layoutSection(environment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection {
        ...
        case .fullWidth(let height):
            section = .init(group: .vertical(
                layoutSize: .init(widthDimension: .fractionalWidth(1), heightDimension: height),
                subitems: [fullItem()]
            ))
        case .selfSizingFullWidth(let estimatedHeight):
            let estimated = NSCollectionLayoutDimension.estimated(estimatedHeight)
            let item = NSCollectionLayoutItem(
                layoutSize: .init(widthDimension: .fractionalWidth(1), heightDimension: estimated)
            )
            section = .init(group: .vertical(
                layoutSize: .init(widthDimension: .fractionalWidth(1), heightDimension: estimated),
                subitems: [item]
        ...
        case .selfSizingCarousel(let width, let estimatedHeight, let scrolling):
            let estimated = NSCollectionLayoutDimension.estimated(estimatedHeight)
            let item = NSCollectionLayoutItem(
                layoutSize: .init(widthDimension: .absolute(width), heightDimension: estimated)
            )
            ...
            section.orthogonalScrollingBehavior = scrolling.collectionViewBehavior
        case .fullWidthPaging(let aspectRatio, let scrolling):
        ...
        case .fixedAspectGrid(let columns, let aspectRatio, let spacing):
            let count = max(columns, 1)
...
extension UICollectionView {
    static func source<SectionID: Hashable & Sendable,ItemIdentifier:Hashable & Sendable>(
        _ collectionView: UICollectionView,
        sections: [Section<SectionID>],
        cellProvider: @escaping UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier>.CellProvider
    ) -> UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier> {
        collectionView.registerSupplementaryViews(for: sections)
        let source = UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier>(
    ...
    static func normalSource<SectionID: Hashable & Sendable,ItemIdentifier:Hashable & Sendable>(
        _ collectionView: UICollectionView,
        cellProvider: @escaping UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier>.CellProvider
    ) -> UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier> {
        let source = UICollectionViewDiffableDataSource<Section<SectionID>, ItemIdentifier>(
