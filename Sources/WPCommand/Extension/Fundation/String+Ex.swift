...
public extension WPSpace where Base == String {
    ...
    func checkingResult(_ patterns:[String],
        ...
        patterns.forEach { key in
            ...
            res.append(contentsOf: regex?.matches(in: base, range: .init(location: 0, length: base.count)) ?? [])
        }
...
public extension WPSpace where Base == String {
    ...
    /// - Parameters:
    ///   - other: 目标字符串
    ///   - options:    .caseInsensitive`      | 忽略大小写
    ///   - options:    .literal`              | 严格按字面匹配（区分大小写和重音符号）
    ///   - options:    .anchored`             | 只在字符串开头匹配
    ///   - options:    .backwards`            | 从尾部开始搜索
    ///   - options:    .diacriticInsensitive` | 忽略重音符号
    ///   - options:    .widthInsensitive`     | 忽略字符宽度差异（全角/半角）
    ///   - options:    .numeric`              | 按数字值比较（自然排序)
    ///   - options:    .regularExpression`    | 使用正则表达式匹配
    /// - Returns: 结果
    func contains(_ other: String?, options: String.CompareOptions = []) -> Bool {
        guard let other = other, !other.isEmpty else { return false }
    ...
    ///   - options: 多行匹配（^ $ 匹配每行） | `.anchorsMatchLines`
    ///   - options: . 能匹配换行符       | `.dotMatchesLineSeparators`
    ///   - options: 正则中允许空格和注释     | `.allowCommentsAndWhitespace`
    ///   - options: 精确匹配特殊字符       | `.ignoreMetacharacters`
    /// - Returns: 结果
    func matches(for pattern: String,
                 options: NSRegularExpression.Options = []) -> [(String, Range<String.Index>)] {
        // 尝试创建正则
        guard let regex = try? NSRegularExpression(pattern: pattern, options: options) else { return [] }
        let nsRange = NSRange(location: 0, length: (base as NSString).length)
        let matches = regex.matches(in: base, options: [], range: nsRange)
        
        let mappedResults:[Any?]  = matches.map { match in
            guard let range = Range(match.range, in: base) else { return nil }
            return (String(base[range]), range)
        }
        return (mappedResults.compactMap({ $0 }) as? [(String, Range<String.Index>)]) ?? []
    }
