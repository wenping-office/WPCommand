...
public extension ObservableType{
             ...
             onDispose: (() -> Void)? = nil) -> Observable<Element>
    {
        return flatMap { elmt in
            ...
            try onNext?(elmt)
            if callBackTypes.contains(where: { $0 == .onNext}){
                callBack(elmt,nil)
            }
        } afterNext: { elmt in
            try afterNext?(elmt)
            if callBackTypes.contains(where: { $0 == .afterNext }) {
                callBack(elmt,nil)
            }
        } onError: { error in
            try onError?(error)
            if callBackTypes.contains(where: { $0 == .onError }) {
                callBack(nil,error)
            }
        } afterError: { error in
            try afterError?(error)
            if callBackTypes.contains(where: { $0 == .afterError }) {
                callBack(nil, error)
            }
        } onCompleted: {
            try onCompleted?()
            if callBackTypes.contains(where: { $0 == .onCompleted }) {
                callBack(nil,nil)
            }
        } afterCompleted: {
            try afterCompleted?()
            if callBackTypes.contains(where: { $0 == .afterCompleted }) {
                callBack(nil,nil)
            }
        } onSubscribe: {
            onSubscribe?()
            if callBackTypes.contains(where: { $0 == .onSubscribe }) {
                callBack(nil,nil)
            }
        } onSubscribed: {
            onSubscribed?()
            if callBackTypes.contains(where: { $0 == .onSubscribed }) {
                callBack(nil,nil)
            }
        } onDispose: {
            onDispose?()
            if callBackTypes.contains(where: { $0 == .onDispose }) {
                callBack(nil,nil)
            }
        }
    }
