...
public extension WPSpace where Base: UIView {
    ...
    var frameInMainWidow: CGRect {
        return base.convert(bounds, to: UIApplication.wp.keyWindow)
    }
    ...
    func removeAllConstraints() {
        if let superview = base.superview {
            for constraint in superview.constraints {
                if constraint.firstItem as? UIView == base || constraint.secondItem as? UIView == base {
...
public extension WPSpace where Base: UIView {
    ...
    func removeFromSuperview(){
        guard let superview = base.superview else {
            NSLayoutConstraint.deactivate(base.constraints)
            return
        }
        let relatedConstraints = superview.constraints.filter { constraint in
            return constraint.firstItem as? UIView == base || constraint.secondItem as? UIView == base
        }
