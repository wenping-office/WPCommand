...
public class WPAutoLatticeView: UIView {
    ...
    public var allViews: [UIView] {
        var newViews = views.wp.elements(where: {!$0.isHidden})
    ...
    /// 重置视图
    func resetSubView() {
        let newViews = allViews.wp.section(with: col)
    ...
    public func clear() {
        contentView.arrangedSubviews.forEach { view in
            ...
            contentView.wp.removeArrangedSubview(view)
        }
        
        views.forEach { view in
            view.wp.removeFromSuperview()
        }
        views = []
    }

    func getRow(_ row: Int) -> UIStackView {
        let target = contentView.arrangedSubviews.wp.get(row)
        if target == nil {
