...
public class AWBubble: UIView {
    
    
    // 气泡
    /// 气泡的填充颜色（背景颜色）
    public var fillColor = AWBubbleConfig.shared.appearance.backgroundColor
    /// 气泡圆角的设置
    public var cornerRadius_aw = AWBubbleConfig.shared.appearance.cornerRadius
    /// 气泡边框颜色
    public var borderColor_aw = AWBubbleConfig.shared.appearance.borderColor
    /// 气泡边框宽度 (设置过大，会出现UI问题)
    public var borderWidth_aw = AWBubbleConfig.shared.appearance.borderWidth
    /// 气泡阴影的颜色
    public var shadowColor_aw: UIColor = AWBubbleConfig.shared.appearance.shadowColor
    /// 气泡阴影的偏移量
    public var shadowOffset_aw: CGSize = AWBubbleConfig.shared.appearance.shadowOffset
    /// 气泡阴影的圆角
    public var shadowRadius_aw: Float = AWBubbleConfig.shared.appearance.shadowRadius
    /// 气泡阴影的不透明度
    public var shadowOpacity_aw: Float = AWBubbleConfig.shared.appearance.shadowOpacity
    /// 气泡的内边距
    public var edgeInsets = AWBubbleConfig.shared.appearance.edgeInsets

    
    // 气泡文字
    /// 气泡上的文字大小
    public var font = AWBubbleConfig.shared.textSetting.font
    /// 气泡上的文字颜色
    public var textColor = AWBubbleConfig.shared.textSetting.textColor
    /// 气泡字体的`NSTextAlignment`
    public var textAlignment = AWBubbleConfig.shared.textSetting.textAlignment
    
    
    
    // 气泡箭头
    /// 气泡箭头的大小
    public var arrowSize = AWBubbleConfig.shared.arrow.arrowSize
    /// 气泡箭头的圆角
    public var arrowRadius = AWBubbleConfig.shared.arrow.arrowRadius
    /// 气泡箭头的偏移量
    public var arrowOffset: ArrowOffset = AWBubbleConfig.shared.arrow.arrowOffset
    ...
    /// 气泡显示动画的类型
    public var entranceAnimation = AWBubble.EntranceAnimation.fadeIn
    ...
    /// 气泡隐藏的动画类型
    public var exitAnimation = AWBubble.ExitAnimation.fadeOut
    /// 自定义结束动画，需要将exitAnimation设置为custom，并且动画结束回调回来。
    public var exitAnimationHandler: ((@escaping () -> Void) -> Void)?
    
    /// 气泡展示中的动画类型
    public var actionAnimation = AWBubble.ActionAnimation.none
    /// 气泡展示中的动画时长
    ...
    /// 点击气泡区域
    public var tapHandler: ((AWBubble) -> Void)?
    /// 点击了气泡之外的区域
    public var tapOutsideHandler: ((AWBubble) -> Void)?
    /// 点击了目标区域
    public var tapTargetHandler: ((AWBubble) -> Void)?
    /// 气泡外滑动
    public var scrollOutsideHandler: ((AWBubble) -> Void)?
    
    
    /// 气泡出现
    public var appearHandler: ((AWBubble) -> Void)?
    /// 气泡消失
    public var dismissHandler: ((AWBubble) -> Void)?
    ...
    /// 气泡展示的方向
    public internal(set) var direction = AWBubble.Direction.auto
    /// 气泡是否正在动画中
    ...
    internal var maxWidth = AWBubbleConfig.shared.maxWidth
    internal var maxWidthBackUp: CGFloat = AWBubbleConfig.shared.maxWidth
    ...
    public func show(text: String, direction: AWBubble.Direction = .autoVertical, maxWidth: CGFloat = AWBubbleConfig.shared.maxWidth, from target: UIView, duration: TimeInterval? = 4) {
        ...
        containerView = UIApplication.wp.keyWindow
        ...
        from = target.wp.convertFrameToScreen()
    ...
    ///   - frame: 目标frame，气泡箭头指向目标的中心。
    ///   - duration: 自动隐藏时间， 默认4秒, 如何设置为nil则不隐藏。
    public func show(attributedText: NSMutableAttributedString, direction: AWBubble.Direction = .autoVertical, maxWidth: CGFloat = AWBubbleConfig.shared.maxWidth, from target: UIView, duration: TimeInterval? = 4) {
        resetView()
    ...
    ///   - frame: 目标frame，气泡箭头指向目标的中心。
    ///   - duration: 自动隐藏时间， 默认4秒, 如何设置为nil则不隐藏。
    public func show(customView: UIView, direction: AWBubble.Direction, from target: UIView, duration: TimeInterval? = 4) {
        resetView()
...
extension AWBubble {
    func setup() {
        ...
        if tapGestureRecognizer == nil {
            tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(AWBubble.handleTap(_:)))
            tapGestureRecognizer?.cancelsTouchesInView = false
            self.addGestureRecognizer(tapGestureRecognizer!)
        }
        if shouldDismissOnTapOutside && tapToRemoveGestureRecognizer == nil {
            tapToRemoveGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(AWBubble.handleTapOutside(_:)))
        ...
        if shouldDismissOnScrollOutside && panGestureRecognizer == nil {
            panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(AWBubble.handleScrollOutside(_:)))
        }
        
        if isApplicationInBackground == nil {
            NotificationCenter.default.addObserver(self, selector: #selector(AWBubble.handleApplicationActive), name: UIApplication.didBecomeActiveNotification, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(AWBubble.handleApplicationResignActive), name: UIApplication.willResignActiveNotification, object: nil)
        }
    }
}
    ...
    fileprivate func textBounds(for text: String?, attributedText: NSMutableAttributedString?, view: UIView?, with font: UIFont, edges: UIEdgeInsets, in maxWidth: CGFloat) -> CGRect {
        ...
        if let attributedText = attributedText {
            ...
            attributedText.enumerateAttribute(.font, in: range) { value, range, isStop in
                if value == nil {
                    attributedText.wp.font(.systemFont(ofSize: 12),range: range)
                }
            }
        ...
        // 最大高度不能超过 屏幕高度的35%
        bounds.size.height = min(bounds.height, AWBubbleConfig.shared.maxHeight)
...
extension AWBubble {
    open override func draw(_ rect: CGRect) {
        ...
        let path = AWBubble.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, arrowRadius: arrowRadius, borderWidth: borderWidth, radius: cornerRadius)
    ...
    fileprivate func updateBubble() {
        ...
        stopActionAnimation {
            UIView.animate(withDuration: 0.2, delay: 0, options: [.transitionCrossDissolve, .beginFromCurrentState], animations: {
                ...
                let path = AWBubble.pathWith(rect: self.frame, frame: self.frame, direction: self.direction, arrowSize: self.arrowSize, arrowPosition: self.arrowPosition, arrowRadius: self.arrowRadius, borderWidth: self.borderWidth, radius: self.cornerRadius)
    ...
    fileprivate func show(duration: TimeInterval? = nil) {
        ...
        performEntranceAnimation {
            ...
            if let duration = duration {
                self.dismissTimer = Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(AWBubble.hide), userInfo: nil, repeats: false)
            }
        }
    }
...
extension AWBubble: UIGestureRecognizerDelegate {
