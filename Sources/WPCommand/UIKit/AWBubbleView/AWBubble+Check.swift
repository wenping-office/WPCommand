...
extension AWBubble {
     ...
     * 尽可能的让气泡的显示趋向屏幕中间
     */
    func checkDirection() -> AWBubble.Direction {
        if direction.isAuto {
            guard let containerView = containerView else { return direction }
            var spaces: [AWBubble.Direction: CGFloat] = [:]
...
extension AWBubble {
    ...
    func checkMaxWidth() -> CGFloat {
        ...
        func check(_ width: CGFloat) -> CGFloat {
            let maxWidth = WPSystem.screen.maxWidth - distanceFromBoundary*2
