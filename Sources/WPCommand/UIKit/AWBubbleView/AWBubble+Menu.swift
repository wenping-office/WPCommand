...
/// 菜单View
public class AWBubbleMenu: UIView {
    
    
    /// 点击选中之后的回调
    public var selectItemBlock: ((AWBubble.Menu.Item) -> Void)?
    ...
    ///   - items: 菜单元素
    ///   - config: 菜单配置
    public init(items: [AWBubble.Menu.Item], config: AWBubble.Menu.Config? = nil) {
        super.init(frame: .zero)
        self.items = items
        self.config = config
        
        frame = getFrame()
        backgroundColor = .red //.color212121
        
        tableView.backgroundColor = .red//.color212121
    ...
    private func getFrame() -> CGRect {
        var tempConfig = AWBubble.Menu.Config()
        if let tem = config {
            tempConfig = tem
        }
        ...
        case .auto(let max):
            ...
            for item in items {
                let text = item.text
                var width: CGFloat = text.wp.width(.systemFont(ofSize: 12), 20)
    ...
    var items: [AWBubble.Menu.Item] = []
    var config: AWBubble.Menu.Config?
}


extension AWBubbleMenu: UITableViewDelegate, UITableViewDataSource {
...
class MenuCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        contentView.backgroundColor = .red//.color212121
