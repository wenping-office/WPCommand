...
extension AWBubble {
    func performActionAnimation() {
        ...
        case .bounce(let offset):
            shouldBounce = true
            bounceAnimation(offset: offset ?? AWBubbleConfig.shared.defaultBounceOffset)
        case .float(let offsetX, let offsetY):
            floatAnimation(offsetX: offsetX ?? AWBubbleConfig.shared.defaultFloatOffset, offsetY: offsetY ?? AWBubbleConfig.shared.defaultFloatOffset)
        case .pulse(let offset):
            pulseAnimation(offset: offset ?? AWBubbleConfig.shared.defaultPulseOffset)
        case .none:
            return
        }
    }
