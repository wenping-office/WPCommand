...
public protocol WPLabelsItemView:UIView{
    ...
    /// 最大的宽
    func labelItemWidth(with data:Data) -> CGFloat
}

public protocol WPLabelsViewDelegate : AnyObject{

    /// 选中某个标签回调
    func labelsView(didSelectAt index: Int,with itemView:any WPLabelsItemView, data: Any)
}
...
public class WPLabelsView<V:WPLabelsItemView>: WPBaseView {
    ...
    /// 数据源
    public private(set) var data : [V.Data] = []
    /// 代理
    public weak var delegate: WPLabelsViewDelegate?
    /// 点击回调
    public var didSelected:((V,V.Data)->Void)?
    ...
    /// 增加标签
    public func append(_ elmt : V.Data){
        data.append(elmt)
        reloadData()
    }

    /// 插入某个标签
    public func insert(_ elmt : V.Data,at:Int){
    ...
    private func resetSubItemsFrame(){
        ...
        for index in 0..<subviews.count {
            let lastX = subviews.wp.get(index - 1)?.wp_maxX ?? 0
        ...
        if alignment == .center || alignment == .right{
            rows.forEach { elmt in
                ...
                for index in 0..<elmt.count {
                    let v = elmt.wp.get(index)
                    let lastV = elmt.wp.get(index - 1)
