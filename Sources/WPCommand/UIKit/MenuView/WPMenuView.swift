...
public class WPMenuView: WPBaseView {
    ...
    override public func observeSubViewEvent() {
        ...
        contentView.bodyView.contentOffSet = {[weak self] x in
            ...
            let bodyItem = self.bodyView.data.wp.get(defaultIndex)
            ...
            var scrollOffSet : CGFloat = Double(x.wp.decimalString()) ?? 0
            // 线条宽度
            ...
            if intX < defaultIndex {
                ...
                if self.contentView.navView.lineView.lineWidth.isLayout{
                    currentLineWidth = self.contentView.navView.lineView.lineWidth.width(with: self.contentView.navView.data.wp.get(defaultIndex)?.navigationItem)
                    targetLineWidth = self.contentView.navView.lineView.lineWidth.width(with: self.contentView.navView.data.wp.get(targetIndex)?.navigationItem)
                ...
                currentX = self.contentView.navView.data.wp.get(defaultIndex)?.navigationItem.superview?.wp_centerX ?? 0
                targetX = self.contentView.navView.data.wp.get(targetIndex)?.navigationItem.superview?.wp_centerX ?? 0
                ...
                if self.contentView.navView.lineView.lineWidth.isLayout{
                    currentLineWidth = self.contentView.navView.lineView.lineWidth.width(with: self.contentView.navView.data.wp.get(targetIndex + 1)?.navigationItem)
                    targetLineWidth = self.contentView.navView.lineView.lineWidth.width(with: self.contentView.navView.data.wp.get(defaultIndex)?.navigationItem)
                    lineSpacing = (currentLineWidth - targetLineWidth) / 100
                    defalutWidth = targetLineWidth
                    lineView.wp_width = defalutWidth + scrollOffSet * lineSpacing * 100
                }
                
                currentX = self.contentView.navView.data.wp.get(targetIndex + 1)?.navigationItem.superview?.wp_centerX ?? 0
                targetX = self.contentView.navView.data.wp.get(defaultIndex)?.navigationItem.superview?.wp_centerX ?? 0
                spacing = (currentX - targetX) / 100
                defaultX = targetX
                lineView.wp_centerX = defaultX + scrollOffSet * spacing * 100
        ...
        contentView.bodyView.didSelected = { [weak self] index in
            ...
            // 设置header头
            let headerItem = self?.contentView.headerView.data.wp.get(index)
    ...
    private func contentViewDidScroll(_ scrollView:UIScrollView){
        ...
        if scrollView.contentOffset.y < contentView.lastOffsetY {
            if let index = currentIndex {
                let item = contentView.bodyView.data.wp.get(index)
    ...
    private func selectedPage(index: Int) {
        ...
        let navItem = contentView.navView.data.wp.get(index)
        ...
        let bodyItem = contentView.bodyView.data.wp.get(index)
        ...
        let headItem = contentView.headerView.data.wp.get(index)
    ...
    private func setChildView(offset:Double,with index:Int){
        ...
        contentView.headerView.data.wp.get(index)?.headerView?.didHorizontalRolling(with: offset)
        contentView.navView.data.wp.get(index)?.navigationItem.didHorizontalRolling(with: offset)
        contentView.bodyView.data.wp.get(index)?.bodyView?.didHorizontalRolling(with: offset)
    }
}
